...
import com.jeequan.task.channel.IRefundService;
...
public class ChannelOrderReissueService {
     ...
     * 处理订单
     **/
    public ChannelRetMsg processPayOrder(PayOrder payOrder, Date startDate, Date endDate) {
    ...
    public ChannelRetMsg processPayOrderBill(PayOrder payOrder, Date startDate, Date endDate) {
        try {
            String payOrderId = payOrder.getPayOrderId();
            //查询支付接口是否存在
            IPayOrderQueryService queryService = SpringBeansUtil.getBean(payOrder.getIfCode() + "PayOrderQueryService", IPayOrderQueryService.class);
